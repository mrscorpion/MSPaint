...
@implementation MSConfigManager{
...
+(id)sharedInstance
{
    static MSConfigManager *mcm = nil;
    
    @synchronized(self)
    {
        if (!mcm)
            mcm = [[MSConfigManager alloc]init];
    }
    return mcm;
}
-(void)setData
...
{
    NSArray *colorArr = [[MSConfigManager sharedInstance] getStandardColorArray];
