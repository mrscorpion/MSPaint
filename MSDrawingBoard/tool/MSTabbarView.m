...
- (void)addTAbbarButtomWithModels:(NSArray *)models{
    for (MSButtonModel *model in models) {
        if (![model isKindOfClass:[MSButtonModel class]]) {
            return;
        }
    }
    for (MSButtonModel *model in models) {
        if (model.isColorSelection) {
            MSColorSelectionButton *button = [MSColorSelectionButton sharedManager];
            button.model = model;
...
- (void)adjustmentButtonFram{
    ...
    for (int i = 0; i<self.buttonArray.count; i++) {
        if (i == 3) {
            MSColorSelectionButton *button = self.buttonArray[3];
