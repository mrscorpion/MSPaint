...
@property (nonatomic,strong) MSPageNormalView *pageView;//用于手绘的图层

@property (nonatomic,strong)MSScratchView *scratchView;//图片显示图层
...
@property (nonatomic,strong) MSColourView *colourView;
...
- (MSColourView *)colourView{
    if (_colourView  ==nil) {
        _colourView = [MSColourView sharedManager];
...
//    _scratchView = [[MSScratchView alloc]initWithFrame:CGRectMake(0, 0, kwidth, kheight)];
//    //涂抹大小
...
//    [self.imageView addSubview:_scratchView];
     ...
     */
    _pageView = [[MSPageNormalView alloc]initWithPageSize:CGSizeMake(kwidth, kheight)];
    ...
    */
    ...
    MSTabbarView *tabbar = [[MSTabbarView alloc]initWithFrame:CGRectMake(0, kheight - 40, kwidth, 40)];
    
    tabbar.backgroundColor = [UIColor clearColor];
    [self.view addSubview:tabbar];
    [tabbar addTAbbarButtomWithModels:self.buttonModels];
    /*
    for (MSButtonModel *model in self.buttonModels) {
...
- (void)donlownDate{
    ...
    [self establishButtonWithImage:@"eraser" slectImage:@"eraser_1" isSelected:NO isColorSelection:NO andAction:^{
        [[MSConfigManager sharedInstance] setFreeInkColorWithIndex:6];
    ...
    [self establishButtonWithImage:@"icon_more" slectImage:@""  isSelected:NO isColorSelection:NO andAction:^{
        MSAboutViewController *vc = [[MSAboutViewController alloc]init];
...
- (void)establishButtonWithImage:(NSString *)image slectImage:(NSString *)slectImage isSelected:(BOOL)selected isColorSelection:(BOOL)isColorSelection andAction:(void(^)(void))action{
    ...
    MSButtonModel *model = [[MSButtonModel alloc]init];
    model.image = image;
